...
void init_6502_sytem(struct system *system)
{
	...
	// LDY  (Zero Page, X) 0xb4 LEN 2 TIM 4
	...
	system->cpu.opcode[0xc0] = { inst_CMY_I, 2, 2 , "CMY" };
	...
	err = fopen_s(&pRomFile, "D:\\Programmierung\\C64 - SID\\asm\\Head_to_Head.dat", "rb");
	...
	fread(&system->bus.mem[0xc0ff], size, 1, pRomFile);
	
	memcpy(&backup_player[0], &system->bus.mem[0xc0ff], 24);
	...
	//system->cpu.reg.pc.value = 0xc0ff;
	system->cpu.reg.pc.value = 0x400;
	time_t t;
...
uint16_t inst_ADC_A(struct system* system, uint8_t* opPara, uint32_t len, uint32_t cyles)
{
	...
	add = system->cpu.reg.a + system->bus.mem[adr.value] + system->cpu.reg.flags.carry;
	system->cpu.reg.a = add;
	// affects C,N,Z,V Flags
	system->cpu.reg.flags.carry = (add & 0x100) != 0;
	update_NZ_Flags(system, system->cpu.reg.a);
...
uint16_t inst_CMP_AY(struct system* system, uint8_t* opPara, uint32_t len, uint32_t cyles)
{
	...
	if (system->cpu.reg.pc.value == 0xe96)
	{
		int halt = 0;
	}
...
uint16_t inst_EOR_I(struct system* system, uint8_t* opPara, uint32_t len, uint32_t cyles)
{
	if (system->cpu.reg.pc.value == 0xe94)
	{
		int halt = 0;
	}
